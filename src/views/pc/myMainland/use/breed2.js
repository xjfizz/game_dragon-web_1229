...
export default function breedHandle() {
    ...
    const breed = async (item, myAddress) => {
        ...
        await getIsApprove(getDpmWithdrawContract)

        await getIsApprove(getDragonMainlandShardsTokenContract)
        loadingHandle(false)
    }
    ...
    const getIsApprove = async (contractFuc) => {
        ...
        try {
            ...
            if (address) {
                ...
                try {
                    ...
                    if (balance <= 0) {
                        if (!breedState.approveForm.dmp.isApprove) {
