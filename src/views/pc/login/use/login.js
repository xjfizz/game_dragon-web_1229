...
import { reactive, onMounted, toRefs, inject, getCurrentInstance, onBeforeUnmount, computed, watch } from "vue";
import { useRouter, useRoute } from "vue-router";
import { initWallet, getSiner } from "@/utils/metaMask/ethers.js";
import { setSession, setToken, getCookie, getSession, setCookie, removeCookie } from "@/utils/auth.js";
...
export default function loginData() {
    ...
    let isLoginInit = computed(() => store.state.login.isLoginInit); // 钱包切换
    ...
    const isLoginHandle = () => {
        ...
        if ((!!sessionStorage.getItem('Token'))) {
            if (JSON.parse(getSession('user')).status != 4 && JSON.parse(getSession('user')).status != 0) {
