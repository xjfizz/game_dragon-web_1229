...
<template>
  <div class="main">
    ...
    <div class="login-wrap">
      ...
      <verificationEmail v-show="loginBoxState == 3" @validEmailCode="validEmailCode"></verificationEmail>
...
export default {
  ...
  setup() {
    ...
    const state = reactive({
      ...
      loginBoxState: 3, // 0: 不弹框  1: 绑定邮箱弹框 2: 注册, 3 验证邮箱验证码
      ...
      API.login.loginSign(params).then((res) => {
        if (res.code == 0) {
          ...
          if (res.data.status == 4) {
            $message.warning(t(`message.pleaseBindEmail`));
      ...
      API.login.loginByEmail(params).then((res) => {
        ...
        if (res.code == 0) {
          ...
          if (res.data.status == 102) {
            $message.warning(t(`message.pleaseBindWalletAddress`));
    ...
    const validEmailCode = (data)=> {
      console.log('validEmailCode', data)
      let params = {
        code: state.metaMaskAddress,
        email:'',
        scene:''
      };
